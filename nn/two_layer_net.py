...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        f1 = X.dot(W1) + b1 # X(N,D) * W1(N,H) = f1(N,H)
        a1 = relu(f1)
        scores = a1.dot(W2) + b2 # a1(N,H) * W2(H,C) = scores(N,C)
        #############################################################################
        ...
        loss = -np.log(probs[np.arange(N), y]) #혹시 이 코드가 각 N에 해당하는 정답 레이블만 읽는건가??
        loss = np.sum(loss)
        ...
        # Backward pass: compute gradients -> 이부분은 배낌
        grads = {}
        #############################################################################
